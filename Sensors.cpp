...
void i2c_init(void) {
  #if defined(ESP32)
    // ESP32 uses Wire library for I2C
    Wire.begin(ESP32_I2C_SDA, ESP32_I2C_SCL);  // SDA=GPIO21, SCL=GPIO22
    Wire.setClock(400000);  // 400kHz I2C clock
    i2c_errors_count = 0;
  #else
    // AVR/Arduino I2C initialization
...
#else
// ESP32 I2C functions using Wire library
void i2c_rep_start(uint8_t address) {
  // ESP32 Wire library handles this automatically
}
...
void initSensors() {
  ...
  #if defined(ESP32) && defined(VBAT)
    // Configure ADC for 1S LiPo battery monitoring (3.0V - 4.2V range)
    // NOTE: For 1S LiPo (4.2V max), use a simple 2:1 voltage divider:
    //   - Two 10kΩ resistors in series (Battery+ -> 10kΩ -> GPIO34 -> 10kΩ -> GND)
    //   - This brings 4.2V down to 2.1V, well within ADC range
    // Alternatively, use 11dB attenuation (0-3.6V) and limit max voltage to 3.5V
    analogReadResolution(12);  // 12-bit ADC (0-4095)
    analogSetAttenuation(ADC_11db);  // 0-3.6V range (or use voltage divider for 0-4.2V)
  #endif
