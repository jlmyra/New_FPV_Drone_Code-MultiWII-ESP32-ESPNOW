...
// Now include EEPROM library after platform is detected
...
// ESP32 EEPROM compatibility wrappers
#if defined(ESP32)
void eeprom_read_block(void *dst, const void *src, size_t n) {
  ...
  uint16_t offset = (uint16_t)(uintptr_t)src;
...
void eeprom_write_block(const void *src, void *dst, size_t n) {
  ...
  uint16_t offset = (uint16_t)(uintptr_t)dst;
  const uint8_t *s = (const uint8_t*)src;
  for (size_t i = 0; i < n; i++) {
    EEPROM.write(offset + i, s[i]);
  }
  EEPROM.commit();  // ESP32 requires commit to save changes
}
#endif
