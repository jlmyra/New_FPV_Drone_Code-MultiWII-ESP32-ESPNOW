...
#elif !defined(ESP32) && !defined(NRF24_RX) && !defined(ESP_NOW_RX) // Standard Channel order (AVR only)
...
void configureReceiver() {
  ...
  #if defined(STANDARD_RX) && !defined(ESP32)
    #if defined(MEGA)
  ...
  /*************************   Special RX Setup   ********************************/
  #endif
  #if defined(SERIAL_SUM_PPM) && !defined(ESP32)
    PPM_PIN_INTERRUPT;
  #endif
  #if defined (SPEKTRUM) || defined(SUMD)
    SerialOpen(RX_SERIAL_PORT,115200);
  #endif
  #if defined(SBUS) && !defined(ESP32)
...
#if defined(PROMICRO) && defined(SERIAL_SUM_PPM) && !defined(ESP32)
  ISR(INT6_vect){rxInt();}
#endif

// PPM_SUM at THROTTLE PIN on MEGA boards
#if defined(PPM_ON_THROTTLE) && defined(MEGA) && defined(SERIAL_SUM_PPM) && !defined(ESP32)
...
/**************************************************************************************/
#if defined(SPEKTRUM) && !defined(ESP32)
void readSerial_RX(void) {
  if ((!f.ARMED) && 
